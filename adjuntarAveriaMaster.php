	...
	if($_FILES["averia"]["error"]>0){
		...
		if(in_array($_FILES["averia"]["type"], $permitidos) && $_FILES["averia"]["size"] <= $limite_kb * 1024){
			...
			if(!file_exists($archivo)){
				...
				$guiaMaster = @move_uploaded_file($_FILES["averia"]["tmp_name"], $archivo);

				if($guiaMaster){
					$sql = "UPDATE `digitalizacion`.`registrovd` SET	`docAveria` = '$archivo' WHERE `idRegistroVD` = '$id'";
