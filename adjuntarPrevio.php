	...
	if($_FILES["previo"]["error"]>0){
		...
		if(in_array($_FILES["previo"]["type"], $permitidos) && $_FILES["previo"]["size"] <= $limite_kb * 1024){
			...
			if(!file_exists($archivo)){
				...
				$resultado = @move_uploaded_file($_FILES["previo"]["tmp_name"], $archivo);

				if($resultado){
					$sql = "UPDATE `digitalizacion`.`vuelodigitalizacion` SET	`docPrevio` = '$archivo' WHERE `idVueloDigitalizacion` = '$id'";
